...
const Menu = () => {
  ...
  return (
    <div className="flex items-center w-full px-8 pt-8 h-[72px] justify-between mt-[25px]">
      <div className="ml-[45px] flex items-center">
        <Link to="/">
          <MirIcon width={48} height={48} color="#fff" />
        </Link>
      </div>
      <nav className="flex flex-row space-x-[30px] p-6 mr-[45px]">
        {menuItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={`menu-link px-6 py-3 text-lg font-bold rounded-md transition-all duration-200 hover:shadow-md no-underline text-white ${
